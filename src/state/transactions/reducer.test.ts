...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.SMARTBCH]).toBeTruthy()
      expect(txs[ChainId.SMARTBCH]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.SMARTBCH]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    it('no op if not valid transaction', () => {
      store.dispatch(
        finalizeTransaction({
          chainId: ChainId.SMARTBCH_AMBER,
          hash: '0x0',
          receipt: {
            status: 1,
            transactionIndex: 1,
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.SMARTBCH_AMBER]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.SMARTBCH), String(ChainId.SMARTBCH_AMBER)])
      expect(Object.keys(store.getState()[ChainId.SMARTBCH] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.SMARTBCH_AMBER] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.SMARTBCH }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.SMARTBCH), String(ChainId.SMARTBCH_AMBER)])
      expect(Object.keys(store.getState()[ChainId.SMARTBCH] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.SMARTBCH_AMBER] ?? {})).toEqual(['0x1'])
    })
  })
})
